...
exports.login = async (req, res) => {
	...
	try {
		const user = await Manager.findOne({ $or: [{ email: username }, { mobile: username }] }).select(
			'password'
		);
		if (!user) {
...
exports.register = async (req, res) => {
	...
	try {
		const _userExists = await Manager.findOne({ $or: [{ email: email }, { mobile: mobile }] });
		if (_userExists) {
			return res.status(400).json({
				success: false,
...
exports.forgotPassword = async (req, res) => {
	...
	try {
		const user = await Manager.findOne({ $or: [{ email: username }, { mobile: username }] });
		if (!user) {
			return res.status(400).json({
				success: false,
				message: 'Invalid Credentials',
...
exports.resetPassword = async (req, res) => {
	...
	try {
		const user = await Manager.findOne({ resetToken });
		if (!user) {
			return res.status(400).json({
				success: false,
				message: 'Password reset failed',
